...
このファイルは、プロフィールの `README.md` を `GitHubReadmeStats` で自動更新するための実践サンプルです。  
`<PATH_TO_GITHUB_README_STATS>` と `<PATH_TO_PROFILE_REPO>` は、あなたの環境に合わせて置き換えてください。

## フルの CLI オプション指定例（ローカル環境で実行する場合）
...
dotnet run --project "$statsRepoPath\src\GitHubReadMeStats.Cli\GitHubReadMeStats.Cli.csproj" --configuration Release -- `
  ...
  --top-languages-image-path ./output/top-languages.svg `
  --github-stats-image-path ./output/github-stats.svg `
  --stats-image-path ./output/stats.svg `
  --public-repo-totals-image-path ./output/public-repo-totals.svg `
  --pins-columns 2 `
...
## `.github/workflows/update-profile-readme-stats.yml` サンプル
...
## README.md の記載例（SVG 出力前）

`README.md` に、先にマーカーだけ置いておきます。  
マーカーがないセクションは自動更新対象になりません。
