...
internal static class GitHubStatsSummaryCardRenderer
{
        ...
        CardColorTheme? colorTheme = null)
    {
        ...
        string backgroundStart = colorTheme?.BackgroundStart ?? "#0D1233";
        ...
        string scoreColor = colorTheme?.TertiaryText ?? "#94A3B8";
        string metaColor = colorTheme?.MutedText ?? "#64748B";
        string gaugeTrackColor = colorTheme?.TrackFill ?? "#312E81";
        string gaugeProgressColor = colorTheme?.AccentStrong ?? "#EC4899";
        ...
        sb.AppendLine("    <style>");
        sb.AppendLine($"      .title {{ font: 700 28px 'Segoe UI', Arial, sans-serif; fill: {EscapeXml(titleColor)}; }}");
        ...
        sb.AppendLine($"  <circle cx=\"{Format(gaugeCenterX)}\" cy=\"{Format(gaugeCenterY)}\" r=\"{Format(gaugeRadius)}\" fill=\"none\" stroke=\"{EscapeXml(gaugeTrackColor)}\" stroke-width=\"7\" />");
        ...
        if (gaugeRatio > 0)
        {
            ...
            sb.AppendLine($"  <path d=\"{gaugePath}\" fill=\"none\" stroke=\"{EscapeXml(gaugeProgressColor)}\" stroke-width=\"7\" stroke-linecap=\"round\" />");
        }
