...
internal static class SvgRenderer
{
        ...
        TimeDisplaySettings timeDisplay)
    {
        ...
        sb.AppendLine("  <text x=\"32\" y=\"42\" class=\"title\">Most Used Languages</text>");
        sb.AppendLine($"  <text x=\"32\" y=\"66\" class=\"sub\">@{EscapeXml(viewerLogin)} | {aggregation.IncludedRepositoryCount} repos | updated {generatedAtLocal:yyyy-MM-dd HH:mm} {EscapeXml(timeDisplay.Label)}</text>");
        ...
        for (int i = 0; i < topLanguages.Count; i++)
        {
            ...
            sb.AppendLine($"  <rect x=\"{chartX}\" y=\"{barY}\" width=\"{chartWidth}\" height=\"22\" rx=\"11\" fill=\"{TrackColor}\" />");
            if (barWidth > 0)
            {
            ...
            sb.AppendLine($"  <text x=\"{chartX + chartWidth - 8}\" y=\"{textY + 1}\" class=\"value\" fill=\"{valueColor}\" text-anchor=\"end\">{language.Percent.ToString("0.00", CultureInfo.InvariantCulture)}% ({ToHumanReadableBytes(language.Size)})</text>");
        }
